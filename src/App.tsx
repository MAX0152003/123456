...
import { checkAttendanceConflict } from './lib/conflictUtils';
import { 
  INITIAL_CLASSES, 
...
import AcademicCertificateModal from './components/AcademicCertificateModal';
import { 
  Wifi, 
  WifiOff, 
  Type, 
...
export default function App() {
    ...
    const unsubscribeAuth = auth.onAuthStateChanged(async (currentUser) => {
      if (currentUser) {
        ...
        try {
          ...
          console.log("Firebase Anonymous sign-in succeeded.");
        } catch (err: any) {
          if (err?.code === 'auth/admin-restricted-operation') {
            console.info("Firebase Anonymous Auth provider is disabled in Firebase Console. Operating in fallback state.");
          } else {
            console.warn("Firebase Anonymous sign-in failed/disabled:", err);
          }
        }
      }
    });
  ...
  // Firestore Sync Data: pulls down classes and attendance records once online + authenticated
  React.useEffect(() => {
    if (isOffline) return;
  ...
  }, [isOffline, !!user, isAuthenticated]);
  ...
  const handleRecordAttendance = (classId: string, status: 'present' | 'late') => {
    ...
    // Check duplicate check-in conflict
    const conflictCheck = checkAttendanceConflict(
      user?.studentId || user?.id || '2023-10492',
      classId,
      attendanceRecords
    );
    ...
    if (conflictCheck.hasConflict) {
      if (typeof window !== 'undefined' && (window as any).showToast) {
        (window as any).showToast(conflictCheck.message, "info");
      }
      speakText(conflictCheck.message, accessibility.readAloud);
      return;
    }
  ...
  const handleResetSystem = async () => {
    ...
    // 5. Purge Firestore Collections to guarantee absolute zero record database state
    if (!isOffline) {
      try {
        const collectionsToPurge = ['classes', 'records', 'messages', 'notifications', 'announcements', 'leaves', 'enrollments'];
        ...
        console.log("All Firestore collections successfully purged.");
      } catch (err) {
  ...
  return (
    ...
    >
          ...
          <div className="flex-1 flex flex-col min-w-0 overflow-hidden h-full">
            ...
            <header className="hidden md:flex px-6 py-4 border-b items-center justify-between gap-4 shrink-0 bg-white/85 dark:bg-zinc-950/80 border-zinc-200 dark:border-zinc-850/80 backdrop-blur-md text-zinc-900 dark:text-zinc-100">
              ...
              <div className="flex items-center gap-2.5">
                ...
                {/* Global Command Palette Quick Launcher */}
                <button
                  onClick={() => setIsCommandPaletteOpen(true)}
                  type="button"
                  className="flex items-center gap-2 px-3 py-1.5 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900 text-zinc-600 dark:text-zinc-300 hover:border-emerald-500/40 text-xs font-semibold transition-all cursor-pointer shadow-xs"
                  title="Search commands and classes (Cmd+K)"
                >
                  <Search className="w-3.5 h-3.5 text-emerald-500" />
                  <span className="hidden lg:inline text-[11px]">Quick Action...</span>
                  <span className="px-1.5 py-0.5 rounded text-[9px] font-mono font-black bg-zinc-200 dark:bg-zinc-800 text-zinc-500 dark:text-zinc-400">
                    ⌘K
                  </span>
                </button>

                {/* Academic Attendance Transcript Certificate Button */}
                <button
                  ...
                  title="Generate Official Attendance Transcript"
                >
            ...
            <main ref={mainScrollRef} className="px-6 md:px-8 pt-2 md:pt-3 pb-24 md:pb-8 max-w-7xl w-full mx-auto space-y-6 flex-1 overflow-y-auto">
      ...
      {/* Global 2026 Command Palette Modal */}
      {user && (
        <CommandPalette
          isOpen={isCommandPaletteOpen}
          onClose={() => setIsCommandPaletteOpen(false)}
          onNavigateTab={(tab) => handleSetScreen(tab)}
          classes={classes}
          userRole={user.role}
          toggleDarkMode={() => setAccessibility(prev => ({ ...prev, theme: prev.theme === 'dark' ? 'light' : 'dark' }))}
          isDarkMode={accessibility.theme === 'dark'}
          speakText={(txt) => speakText(txt, accessibility.readAloud)}
        />
      )}

      {/* Official Verifiable Academic Transcript Modal */}
      {user && (
