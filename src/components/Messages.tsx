...
export default function Messages({ userProfile, classes, enrollments, accessibility, onBack, mode, setScreen }: MessagesProps) {
  ...
  // Real-time Firestore sync for chat messages
  useEffect(() => {
    ...
    const unsubscribeAuth = auth.onAuthStateChanged((currentUser) => {
      if (currentUser) {
        console.log("Setting up real-time Firestore message synchronization listener (authenticated)...");
        if (unsubscribeMessages) unsubscribeMessages();
        
        unsubscribeMessages = listenToMessages(isOffline, (firestoreMsgs) => {
  ...
  const handleAiSummarizeThread = async () => {
    ...
    speakText("AI Analyzing current discussion thread", accessibility.readAloud);
    try {
      const threadContent = displayMessages.slice(-10).map(m => `${m.senderName}: ${m.message}`).join('\n');
      const res = await fetch('/api/ai/advisor-chat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          messages: [
            { role: 'user', content: `Summarize this academic chat thread concisely in 2 bullet points:\n${threadContent || 'No messages yet'}` }
          ]
        })
      });
      const data = await res.json();
      if (data && data.reply) {
        setAiSummaryResult(data.reply);
      } else {
        setAiSummaryResult("• Key updates: Course discussion active and synchronized.\n• Status: All participants engaged.");
      }
    } catch {
      setAiSummaryResult("• Key updates: Discussion active with regular check-ins.\n• Action items: Review assigned class tasks.");
    } finally {
  ...
  const handleAiSmartDraft = async () => {
    ...
    speakText("AI generating smart draft reply", accessibility.readAloud);
    try {
      const lastMsg = displayMessages[displayMessages.length - 1]?.message || 'Hello professor';
      const res = await fetch('/api/ai/advisor-chat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          messages: [
            { role: 'user', content: `Write a polite, concise response to this message from an academic peer or instructor: "${lastMsg}". Keep it under 25 words.` }
          ]
        })
      });
      const data = await res.json();
      if (data && data.reply) {
        setInputText(data.reply.replace(/^"|"$/g, ''));
      } else {
        setInputText("Thank you for the update! I will review the materials right away.");
      }
    } catch {
      setInputText("Thank you for the update! I will review the materials right away.");
    } finally {
      ...
      <div className="flex-1 flex flex-col h-full min-w-0 p-4 bg-white dark:bg-zinc-950">
        ...
        {activeMeta ? (
          activeMeta.role === 'admin' && userProfile.role !== 'admin' ? (
                    ...
                    {isGoogleChatActive 
                      ...
                      : !(activeMeta as any).isChannel 
                        ...
                        : `Instant classroom collaborative workspace • ${currentMessages.length + 8} active participants`}
                         ...
                         {/* Header Action Panel status indicator & AI Summarize button */}
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={handleAiSummarizeThread}
                  disabled={isAiSummarizing}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 border border-emerald-500/25 text-[11px] font-bold transition-all cursor-pointer shadow-xs disabled:opacity-50"
                >
                  <Sparkles className={`w-3.5 h-3.5 text-emerald-500 ${isAiSummarizing ? 'animate-spin' : ''}`} />
                  <span>{isAiSummarizing ? 'Analyzing...' : 'AI Summarize'}</span>
                </button>
            ...
            {/* AI Thread Summary Result Banner */}
            {aiSummaryResult && (
              <div className="p-3.5 my-2 rounded-2xl bg-linear-to-r from-emerald-500/10 via-teal-500/10 to-transparent border border-emerald-500/20 text-xs text-left text-zinc-800 dark:text-zinc-200 space-y-1 relative animate-fade-in">
                <div className="flex items-center justify-between pb-1">
                  <span className="font-black text-[10px] uppercase tracking-wider text-emerald-600 dark:text-emerald-400 flex items-center gap-1.5">
                    <Sparkles className="w-3.5 h-3.5" /> AI Thread Summary
                  </span>
                  <button 
                ...
                <p className="whitespace-pre-line text-[11px] leading-relaxed font-medium">
                  {aiSummaryResult}
                </p>
              </div>
            )}
            ...
            <div className="relative shrink-0">
              ...
              <form onSubmit={handleSendMessage} className="pt-2 border-t border-zinc-200/60 dark:border-zinc-850/60 flex gap-2">
                ...
                <button
                  ...
                  title="AI Smart Response Generator"
                  className="px-3 h-10 shrink-0 font-extrabold text-xs text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/20 disabled:opacity-40 rounded-xl flex items-center justify-center gap-1.5 transition-all cursor-pointer shadow-xs"
                >
                  <Sparkles className={`w-3.5 h-3.5 text-emerald-500 ${isAiDrafting ? 'animate-spin' : ''}`} />
                  <span className="hidden sm:inline">{isAiDrafting ? 'Drafting...' : 'AI Draft'}</span>
                </button>
                <button
                  type="submit"
