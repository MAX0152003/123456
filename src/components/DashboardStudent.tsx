  ...
  const handleDownloadReport = () => {
    ...
    if (studentRecs.length === 0) {
      if (typeof window !== 'undefined' && (window as any).showToast) {
        (window as any).showToast("No attendance logs found to export for this student.", "warning");
      }
    ...
    <div className="space-y-6">
      ...
      <AnimatePresence mode="wait">
        ...
        {activeScreen === 'dashboard' && (
          ...
          {/* Prominent Live Instructors Directory */}
          <InstructorsDirectory
            classes={classes}
            facultyStatuses={facultyStatuses}
            onMessageInstructor={(id, name) => {
              setScreen('messages');
              speakText(`Navigating to direct message with ${name}`, accessibility.readAloud);
            }}
            onOpenAdvisor={() => setIsAdvisorOpen(true)}
          />
